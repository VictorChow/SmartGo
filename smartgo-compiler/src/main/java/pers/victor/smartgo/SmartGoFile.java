...
class SmartGoFile {
    ...
    static void createSmartGo(Filer filer) throws Exception {
        ...
        for (Map.Entry<String, SmartGoEntity> entry : map.entrySet()) {
            ...
            for (SmartGoEntity.FieldEntity field : entry.getValue().fields) {
                ...
                MethodSpec method = MethodSpec.methodBuilder(methodName)
                        ...
                        .returns(ClassName.get(SmartGoProcessor.packageName, "To" + className))
            ...
            MethodSpec method = MethodSpec.methodBuilder("to" + className)
                    ...
                    .returns(ClassName.get(SmartGoProcessor.packageName, "To" + className))
                    .addStatement("return new $T()", ClassName.get(SmartGoProcessor.packageName, "To" + className))
                    .build();
        ...
        createToActivityClass(goToActivitiesMethodList, filer);
        createToTargetActivityClass(targetActivitiesClassList, filer);
        ...
        MethodSpec from = MethodSpec.methodBuilder("from")
                ...
                .returns(ClassName.get(SmartGoProcessor.packageName, "ToActivity"))
                .addStatement("context = ctx")
                .addStatement("intent = new Intent()")
                .addStatement("return new $T()", ClassName.get(SmartGoProcessor.packageName, "ToActivity"))
                .build();
        //SmartGo类里go()
        MethodSpec go = MethodSpec.methodBuilder("go")
     ...
     * 生成ToActivity.java
     */
    private static void createToActivityClass(List<MethodSpec> list, Filer filer) throws Exception {
        ...
        TypeSpec type = TypeSpec.classBuilder("ToActivity")
                ...
                .addMethods(list)
                .build();
        JavaFile.builder(SmartGoProcessor.packageName, type).build().writeTo(filer);
    }

    /**
     * 生成ToXXXActivity.java
     */
    private static void createToTargetActivityClass(List<TypeSpec> list, Filer filer) throws Exception {
        for (TypeSpec spec : list) {
            JavaFile.builder(SmartGoProcessor.packageName, spec).build().writeTo(filer);
        }
    }

    private static TypeName getFieldType(SmartGoEntity.FieldEntity field) {
        TypeName typeName;
    ...
    static void createInjectors(Filer filer) throws Exception {
        for (Map.Entry<String, SmartGoEntity> entry : map.entrySet()) {
            String fullClassName = entry.getKey();
    ...
    private static void getExtras(MethodSpec.Builder builder, SmartGoEntity.FieldEntity field) {
    ...
    static void getEachVariableElement(Element element, ProcessingEnvironment processingEnv) {
